...
Mesh* MeshFactory::CreateTestModel()
{
	...
	Mesh* pMesh = new Mesh();
	InvertFaces(VertexIndices.data(), (unsigned int)VertexIndices.size());
	// pMesh->CalculateUVCoords(t_VertexData);
	pMesh->BufferMeshData((int)t_VertexData.size(), t_VertexData.data(), (int)VertexIndices.size(), VertexIndices.data());
...
Mesh* MeshFactory::CreateCircle(float radius, unsigned int points, vec2 UVScale)
{
	...
	{
		indices.push_back(0); // #NOTE Center first
	...
	// generate UVCOORDS
	CalculateXZUVCoords(verts.data(), (int)verts.size());

	if (UVScale != vec2(1.0f, 1.0f))
	{
		ScaleUVCOORDS(verts.data(), (int)verts.size(), UVScale);
	}
